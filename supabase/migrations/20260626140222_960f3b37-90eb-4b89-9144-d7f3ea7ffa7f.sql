...
BEGIN
  ...
  LOOP
    ...
    IF _produto IS NULL OR _valor IS NULL OR _valor <= 0 THEN
      RETURN QUERY SELECT _produto, 'skipped_invalid'::text, 'produto/valor ausente'::text;
      CONTINUE;
    END IF;

    IF _unidade_id IS NULL THEN
      RETURN QUERY SELECT _produto, 'erro'::text, 'unidade obrigatória'::text;
      CONTINUE;
    END IF;
    ...
    IF _has_manual THEN
      RETURN QUERY SELECT _produto, 'skipped_manual'::text, NULL::text;
      CONTINUE;
    END IF;
